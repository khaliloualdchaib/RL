    ...
    with open(log_file, "w") as f:
        for iteration in range(num_iterations):
            ...
            for perturbed_params in perturbed_params_list:
                score = sum(evaluate_policy(env=env, policy_net=policy_net, params=perturbed_params, num_episodes=episodes_per_eval))/episodes_per_eval
                if score > best_score:
                    best_score = score
            ...
            evaluation_rewards = evaluate_policy(env=env, policy_net=policy_net, params=best_params, num_episodes=episodes_per_eval, discount_factor=discount_factor)
