...
def zeroth_order_method(env, policy_net, num_iterations=1000, learning_rate=0.00001, log_file="zeroth_order_log.txt", decay_rate=0.99, num_episodes_per_eval=20, discount_factor=0.99):
    with open(log_file, "w") as f:
        ...
        for i in range(num_iterations):
            ...
            theta_plus_score = sum(evaluate_policy(env=env, policy_net=policy_net, params=theta_plus, num_episodes=num_episodes_per_eval, discount_factor=discount_factor))/num_episodes_per_eval
            theta_min_score = sum(evaluate_policy(env=env, policy_net=policy_net, params=theta_min, num_episodes=num_episodes_per_eval, discount_factor=discount_factor))/num_episodes_per_eval
            ...
            evaluation_rewards = evaluate_policy(env=env, policy_net=policy_net, params=current_params, num_episodes=num_episodes_per_eval, discount_factor=discount_factor)
